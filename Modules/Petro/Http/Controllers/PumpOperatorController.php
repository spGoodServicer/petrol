    ...
    public function index()
    {
        ...
        if (request()->ajax()) {
            ...
            if (request()->ajax()) {
                ...
                return $fuel_tanks->rawColumns(['action', 'sold_fuel_qty', 'sale_amount_fuel', 'excess_amount', 'short_amount', 'commission_rate', 'commission_amount', 'current_balance'])
                    ->make(true);
            }
        }
    ...
    public function update($id, Request $request)
    {
        ...
        try {
            ...
            Transaction::where([
                ...
                'pump_operator_id' => $pump_operator->id,
                'business_id' => $business_id
            ])->update(['transaction_date' => \Carbon::parse($request->transaction_date)]);
    ...
    public function getLedger()
    {
        ...
        foreach ($ledger_transactions as $transaction) {
            if ($transaction->acc_transaction_type === 'debit') {
                $ledger_details['total_short'] += $transaction->amount;
            }

            if ($transaction->acc_transaction_type === 'credit') {
                $ledger_details['total_recovered_excess'] += $transaction->amount;
            }
        }

        $ledger_details['balance_due'] = ($ledger_details['beginning_balance'] + $ledger_details['total_short']) - $ledger_details['total_recovered_excess'];
