...
class ReportController extends Controller
{
    ...
    {
        $business_id = $request->session()->get('user.business_id');
        if (!$this->moduleUtil->hasThePermissionInSubscription($business_id, 'verification_report') && !$this->moduleUtil->hasThePermissionInSubscription($business_id, 'report_verification')) {
            abort(403, 'Unauthorized action.');
        }
        $business_locations = BusinessLocation::forDropdown($business_id);
        $petro_module = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'enable_petro_module');
                ...
                ->addColumn(
                    ...
                    function ($row) {
                        ...
                        if (auth()->user()->can("sell.view") || auth()->user()->can("direct_sell.access") || auth()->user()->can("view_own_sell_only")) {
                            $html .= '<li><a href="#" data-href="' . action("SellController@showReport", [
                                'ref_no' => $row->payment_ref_no,
                                'paid_on' => $row->paid_on
                            ]) . '" class="btn-modal" data-container=".view_modal"><i class="fa fa-external-link" aria-hidden="true"></i> ' . __("messages.view") . '</a></li>';
                        }
    ...
    public function getDailyReport(Request $request)
    {
        ...
        $total_interest_outstanding = $this->getTotalInterestOutstanding($business_id, $start_date, $end_date, $location_id, false);
        ...
        $outstandings['received'] = $total_received_outstanding -$total_interest_outstanding;
    ...
    public function getMonthlyReport(Request $request, $is_previous = false)
    {
        ...
        $petro_module = $this->moduleUtil->hasThePermissionInSubscription($business_id, 'enable_petro_module');
        $sub_categories = Category::leftjoin('categories as parent_cat', 'categories.parent_id', 'parent_cat.id')->where('categories.business_id', $business_id)
                                  ->where('categories.parent_id', '!=', 0)
                                  ->select('categories.*')->get();     //Category::subCategoryOnlyFuel($business_id);
        $cash_account = $this->transactionUtil->account_exist_return_id('Cash');
        $total_sales = [];
        $sub_category_sales = [];
        $received_payments = [];
