$(document).ready(function () {
    ...
    $('#search_product')
        ...
        var is_overselling_allowed = false;
        if ($('input#is_overselling_allowed').length > 0) {
            is_overselling_allowed = true;
        }
            ...
            success: function (result) {
                if (result.success == 1) {
                    ...
                    if (result.receipt.is_enabled) {
                        pos_print_invoice(result.receipt);
                    }
...
function pos_product_row(variation_id, purchase_line_id = null) {
    ...
    if ($('#item_addition_method').length) {
        addtn_method = $('#item_addition_method').val();
    }

    if ($('#service_addition_method').length) {
        addtn_method = $('#service_addition_method').val();
    }
    if (addtn_method == 0) {
        add_via_ajax = true;
    } else {
...
function reset_pos_form() {
    ...
    set_default_customer();
    set_location();
    getStores($("select#select_location_id option").filter(":selected").val());
...
function pos_print_invoice(receipt) {
    //If printer type then connect with websocket
    if (receipt.print_type == 'printer') {
        var content = receipt;
        content.type = 'print-receipt';
...
function getStores(location_id) {
    console.log('change#select_location_id');
    $.ajax({
        method: 'get',
        ...
        contentType: 'html',
        success: function(result) {
            $('#select_store_id').empty().append(result);
        },
    });
}